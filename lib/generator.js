...
function generateSvg(files, options) {
  return new Promise((resolve, reject) => {
    ...
    if (options.height) {
      opt.fontHeight = options.height;
    }
    if (options.centerHorizontally) {
      opt.centerHorizontally = options.centerHorizontally;
    }
    const fontStream = new SVGIcons2SVGFontStream(opt)
...
function dest(options) {
  ...
  options.types.forEach((type) => {
    const dest = path.join(`${options.outDir}/${options.fontName}.${type}`);
